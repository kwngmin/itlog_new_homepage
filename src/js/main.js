    ...
    if (count) {
        hexList.forEach((el, idx) => {
            if (el.classList.contains('aos-animate')) {
                ...
                countList[idx] && counter(countList[idx], countList[idx].getAttribute('data-count'), idx, countList[idx].getAttribute('data-time'));
            }
        })
    }
});
...
let mainSwiper = new Swiper('.mainSwiper-container', {
            pagination: {
                ...
                renderBullet: function (index, className) {
                    return '<span class="' + className + '"><small class="hide768">' + (keyword[index]) +
                        '</small></span>';
                },
            },
            loop: true,
...
const responsiveBullet = () => {
    document.querySelectorAll('.mainSwiper-container .swiper-pagination-bullet').forEach(el => {
...
const swiper = new Swiper('.productSwiper-container', {
    ...
    loop: true,
    pagination: {
        el: ".productSwiper-container .swiper-pagination",
        clickable: true,
        // dynamicBullets: true,
    },
    breakpoints: {
...
// 건설사 이미지 셋팅
const partnership = ['dar', 'doosan2', 'dw', 'han', 'hs', 'hy', 'lotte', 'posco'];
